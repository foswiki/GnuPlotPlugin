...
"GnuPlotPlugin::PlotSettings::fromFile - Creating new PlotSettings Object from file for $web / $topic / $plotName"
    );
    my $self = {};
    $self->{WEB}   = $web;
    $self->{TOPIC} = $topic;
...
sub readFile {
    my $gnuFile = $_[0];
    TWiki::Func::writeDebug(
        "GnuPlotPlugin::PlotSettings::readFile - Reading settings from $gnuFile"
    ) if $debug;
...
sub writeFile {
    ...
    TWiki::Func::writeDebug(
"GnuPlotPlugin::PlotSettings::writeFile - Writing ---=$text=--- to $gnuFile"
    );    # if $debug;
...
sub newFile {
    TWiki::Func::writeDebug(
        "GnuPlotPlugin::PlotSettings::newFile - Creating new default settings")
      if $debug;
    my $text = '';
...
sub render {
    ...
    $text .=
      ...
      . "\#gnuplot$self->{NAME}\" method=\"post\">\n";
    $text .= "<table>\n";
    $text .= "  <tr valign=\"middle\">\n";
    $text .=
"    <td><textarea  rows=\"10\" cols=\"90\" name=\"gnuPlotSettingsText\" >$self->{TEXT}</textarea>\n";
    $text .= "    </td>\n";
    $text .=
"    <td><input  type=\"submit\" value=\"Save Settings\" class=\"twikiSubmit\" /><br>\n";
    $text .=
"        <a target=\"GnuPlotPlugin\" onclick=\"return launchWindow('TWiki','GnuPlotPlugin')\" href=\"/twiki/bin/view/TWiki/GnuPlotPlugin\">GnuPlotPlugin help</a><br>\n";
    $text .=
"        <a target=\"GnuPlotHelp\" onclick=\"return launchWindow('TWiki','GnuPlotHelp')\" href=\"/twiki/bin/view/TWiki/GnuPlotHelp\">Gnuplot help</a>\n";
    $text .= "    </td>\n";
    $text .= "  </tr>\n";
    $text .= "</table>\n";
    $text .=
