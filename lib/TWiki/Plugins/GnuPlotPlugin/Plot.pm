...
sub new {
    ...
    TWiki::Func::writeDebug(
        "GnuPlotPlugin::Plot::new - Creating new Plot with name $plotName")
      if $debug;
    my $self = {};
    $self->{WEB}     = $web;
    $self->{TOPIC}   = $topic;
...
sub render {
    my $self = shift;
    TWiki::Func::writeDebug(
        "GnuPlotPlugin::Plot::render - Rendering $self->{NAME}")
      if $debug;
  ...
  # Update $gnuplotPath, $gnuplotHelperPath and $execCmd to fit your environment
        ...
        my $execCmd =
"/usr/bin/perl %HELPERSCRIPT|F% %GNUPLOT|F% %WORKDIR|F% %INFILE|F% %OUTFILE|F% %ERRFILE|F% ";
        TWiki::Func::writeDebug(
            "GnuPlotPlugin::Plot::render - GnuPlot path: $gnuplotPath")
          if $debug;
        TWiki::Func::writeDebug(
"GnuPlotPlugin::Plot::render - GnuPlot helper path: $gnuplotHelperPath"
        ) if $debug;
        TWiki::Func::writeDebug(
            "GnuPlotPlugin::Plot::render - Executing $execCmd in sandbox")
          if $debug;
        TWiki::Func::writeDebug(
            "GnuPlotPlugin::Plot::render - errorfile set to $errFile")
          if $debug;
        ...
        TWiki::Func::writeDebug(
            "gnuplot-sandbox: output $output status $status")
          if $debug;
        ...
        if ( -s $pngFile ) {
            ...
            $renderedText .=
"*GnuPlotPlugin Error:* Cannot display the plot because the image file ($self->{PNGFILE}) has zero size. With a bit of luck the reason for this will be shown below.\n---\n";
        }
        if ( -s $errFile ) {
        ...
        TWiki::Func::writeDebug(
            "GnuPlotPlugin::Plot::render - $gnuFile does not exist")
          if $debug;
        $renderedText =
"No settings found for this plot (<nop>$self->{GNUFILE} not found). Click on the Edit button below to generate and edit the settings for this plot.\n";
...
sub parseFile {
    ...
    TWiki::Func::writeDebug(
        "GnuPlotPlugin::Plot::readFile - Reading settings from $gnuFile")
      if $debug;
...
sub editPlotSettingsButton {
    ...
    $text .=
      ...
      . "' method=\"post\" >\n";
    $text .=
"<input type=\"hidden\" name=\"gnuPlotName\" value=\"$self->{NAME}\" />\n";
    $text .=
      "<input type=\"hidden\" name=\"gnuPlotAction\" value=\"edit\" />\n";
    $text .=
"<input type=\"submit\" value=\"Edit Plot Settings\" class=\"twikiSubmit\"></input>\n";
    $text .= "</form>\n";
    return $text;
}
